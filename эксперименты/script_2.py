...
class ParserDELL:
    ...
    def request(self, model, url):
        ...
        for osys in os_code:
            ...
            req = requests.get(link, headers=headers0, verify=False, auth=auth)  # requests.get(link)
            js = req.json()
        ...
        with open(file_to_save, encoding='utf-8', mode='w') as f:  # открыли файл для записи w - запись, если файла нет
            ...
            for json_driver in list_json:
                ...
                for i in range(count_driver_cat):
                    ...
                    dict_for_base = {}
                    dict_for_base['meta_k'] = "скачать,драйверы,бесплатно,на высокой скорости,dell,inspiron,3521,windows 7,64 bit"
                    dict_for_base['meta_d'] = "Скачать бесплатно драйверы на высокой скорости для Dell %s [%s]" % (model, osys)
                    dict_for_base['title'] = "%s %s" % (model, osys)
