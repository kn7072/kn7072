...
чтобы заменить символ | на ``` и нажатие enter
g/\|/s/\v\|(.*)\|/```^M\1^M```
...
```
1. скопировать все заголовки в конец файла(то что получилось убрать временно в другой файл)
если в файле есть комментари решетки, тогда выбираем только те после которых есть пустые строки
g/\v^(#{1,6} )(.*)\n{2,}/copy$

простая версия
g/\v^(#{1,3})(.*)/copy$

2. добавить ко всем заголовкам ссылки - номер генерируются автоматически
выбираем только те заголовки после которых есть пустые строки
let @a=1 | g/\v^(#{1,5} )(.*)\ze(\n{2,})/s//\=submatch(1).submatch(2).' <a name="link_'.(@a+setreg('a',@a+1)).'"><\/a>'/g
старая версия
let @a=1 | g/\v^(#{1,5} )(.*)/s//\=submatch(1).submatch(2).' <a name="link_'.(@a+setreg('a',@a+1)).'"><\/a>'/g

3. заменяем крывые закрывающие теги \/a на /a
%s#\\/a#/a#g

4. возвращаемся к заголовкам из пункта 1 - они должны находится в отдельном буфере (для простоты работы и анализа), применяем две команды к ним (можно и одной - нужно проверить)
