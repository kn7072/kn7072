...
docker run -v путь*локального*каталога(host машины ):путь*в*контейнере nginx
...
посмотреть запущенные службы в сетевом пространстве, где 5335 сид nginx(как пример)
nsenter -t 5335 -n ss -tupln

или подключить оболочку выполнять разные команды
nsenter -t 5335 -n bash
ss -tupln
ip route show
