...
local menu_terminal = {"open terminal", terminal, beautiful.terminal_icon} -- /usr/share/icons/hicolor/128x128/apps
local menu_macmillan = {
    "macmillan", "flatpak run io.github.xiaoyifang.goldendict_ng"
}
local firefox_app = "firefox"
mystuff = {{"Web Browser", firefox_app}, {"Another Browser", "google-chrome"}}

if has_fdo then
    mymainmenu = freedesktop.menu.build({
        before = {menu_awesome},
        after = {menu_terminal, menu_macmillan, {"browsers", mystuff}}
    })
else
...
firefox_button:buttons(gears.table.join(firefox_button:buttons(), awful.button(
                                            {}, 1, nil, function()
        awful.spawn(firefox_app)
    end)))
...
local keyboard_layout = require("keyboard_layout2")

-- print(package.searchpath('keyboard_layout', package.path))
-- naughty.notify({
--     title = "keyboard_layout path",
--     text = package.searchpath('keyboard_layout', package.path),
--     fg = "#ff0000",
--     bg = "#deb887",
--     timeout = 15,
--     position = "bottom_left"
...
kbdcfg.add_primary_layout("English", "US", "us,ru")
kbdcfg.add_primary_layout("Русский", "RU", "ru,us")
kbdcfg.bind()
kbdcfg.widget:buttons(awful.util.table.join(
                          awful.button({}, 1,
                                       function() kbdcfg.switch_next() end),
                          awful.button({}, 3,
                                       function() kbdcfg.menu:toggle() end)))
...
awful.screen.connect_for_each_screen(function(s)
    ...
    awful.tag({"internet", "prog", "eng", "media", "other"}, s, {
        awful.layout.layouts[1], awful.layout.layouts[7],
        awful.layout.layouts[7], awful.layout.layouts[7],
    ...
    s.mywibox_2 = awful.wibar({position = "left", screen = s, width = 30})
...
-- }}}
-- awful.screen.connect_for_each_screen(function(s) -- Create a tasklist widget
--     s.mytasklist = awful.widget.tasklist {
--         screen = s,
--         filter = awful.widget.tasklist.filter.currenttags,
--         buttons = tasklist_buttons
...
-- Shift-Alt to change keyboard layout
                                   awful.key({"Shift", "Control"}, "Alt_L",
                                             function() kbdcfg.switch_next() end), -- Alt-Shift to change keyboard layout
