...
def create_json_word(word_i):
    ...
    temp_list_words.append(
        [
            word_i,
            translate,
            transcription,
            dict_examples["examples_eng"],
            dict_examples["examples_rus"],
        ]
    )
...
def update_word_dict():
    ...
    temp_dict = {}
    for word_i, translate, transcription, examp_eng, examp_rus in temp_list_words:
        copy_temp = deepcopy(examples_keys)
        copy_temp["transcription"] = transcription
        copy_temp["translate"] = translate
        copy_temp["examples"] = examp_eng
        copy_temp["example_translate"] = examp_rus
        temp_dict[word_i] = copy_temp
